...
# def H(tau):
#     if tau>5/timestep:
#         return np.exp(-tau/100/timestep)
#     elif tau<-5:
#         return -np.exp(tau/100/timestep)
#     else:
#         return 0
...
class webotsDriver (Supervisor):
    ...
    def initialization(self, context, mode, randomize=False):
        ...
        self.landmarks = np.inf * np.ones((5, 1))   # tf.Variable(tf.zeros((5, 1)))
        self.boundaries = tf.Variable(tf.zeros(720, 1))
        self.act = tf.zeros(self.n_hd)
        self.step(self.timestep)
        self.goalLocation = [[-1.2, 1.2], [-1.5, -3], [-3, 2.75], [-1, -1]][context] # [1, .9]
        self.expectedReward = 0
        self.lastReward = 0
        self.context = context
        self.s = tf.zeros_like(self.pcn.v)
        self.s_prev = tf.zeros_like(self.pcn.v)
        # self.trans_prob = tf.identity(self.pcn.w_rec)

        if randomize:
            INITIAL = [rng.uniform(-1.2192, 1.2192), 0.5, rng.uniform(-1.2192, 1.2192)]
        ...
        #     hmap_z = np.zeros((num_steps, num_pc))
        #     hmap_h = np.zeros((num_steps, n_hd))
        #     hmap_g = np.zeros((num_steps, num_gcs**2))
    ...
    def atGoal(self, exploit, s=0):
        ...
        #HERE
        print()
        #if self.getTime() >= 60*self.runTime:
        # #     return
        print(f"curPos {currPos[0], currPos[2]} and goal is {self.goalLocation}")
        ...
        if (self.mode=="dmtp" \
            ...
            or ((self.mode=="cleanup" or self.mode=="learning") and (self.getTime() >=60*self.runTime)): #   # and   #
            print("Made it")
            ...
            plot.show()
            self.save(True) # not(exploit)) #or self.mode=="dmtp"))
            self.worldReload()
